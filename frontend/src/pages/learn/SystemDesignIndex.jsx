...
import { Clock, ChevronRight, BookOpen, Zap, Users, ArrowRight, ArrowLeft, HelpCircle, Lock, Crown } from 'lucide-react';
        ...
        <div className="container max-w-5xl mx-auto px-4">
          ...
          <div className="text-center mb-14">
            {/* <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#06b6d4]/10 border border-[#06b6d4]/30 mb-6">
              ...
              <span className="text-sm font-semibold text-[#06b6d4]">60% free — Premium topics from ₹499</span>
            </div> */}
            ...
            <div className="flex flex-wrap items-center justify-center gap-6 text-sm mb-8">
              {[
                ...
                { icon: Users, text: 'Free with login' },              ].map((item, i) => (
          ...
          {!user ? (
          <>
          {/* Show all sections with locks for non-logged-in users */}
                ...
                <div className="flex items-center gap-3 mb-4 flex-wrap">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {section.access === 'free' && <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--green-bg)] text-[var(--green)] font-semibold">Free</span>}
                      {section.access === 'pro' && <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--blue-bg)] text-[var(--blue)] font-semibold">Pro</span>}
                      {section.access === 'elite' && <span className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--purple-bg)] text-[var(--purple)] font-semibold">Elite</span>}
                    </div>
                  ...
                  {sortByDifficulty(section.topics, TOPICS).map((slug) => {
                    ...
                    return (
                      <Link
                        ...
                        className={`flex items-start gap-3 p-4 rounded-xl ${theme.bg.card} border ${theme.border.primary} opacity-60 transition-all hover:opacity-80`}
                      >
                        <div className="flex-1 min-w-0">
          ...
          <div className={`mt-10 p-8 rounded-2xl ${theme.bg.card} border-2 border-[#06b6d4]/20 text-center`}>
            ...
            <h3 className={`text-xl font-bold ${theme.text.primary} mb-2`}>Sign in to start learning</h3>
            <p className={`${theme.text.secondary} mb-6 text-sm max-w-lg mx-auto`}>
              The Quick Start section (6 topics) is free for all logged-in users. Upgrade to Pro or Elite to unlock Fundamentals, Interview Questions, and Advanced topics.
            </p>
          ...
          {/* Premium CTA for non-elite logged-in users */}
          {user && user?.plan_id !== 'elite' && (
          <div className={`mt-14 p-8 rounded-2xl ${theme.bg.card} border-2 border-amber-500/20 text-center`}>
            ...
            <h3 className={`text-xl font-bold ${theme.text.primary} mb-2`}>Unlock all 50+ topics</h3>
            <p className={`${theme.text.secondary} mb-6 text-sm max-w-lg mx-auto`}>
              You have access to the free Quick Start section. Upgrade to unlock Fundamentals, all Interview Question breakdowns, Design Patterns, and Advanced topics.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <Link to="/apply" className="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-semibold rounded-xl hover:from-amber-600 hover:to-orange-600 transition-all duration-200">
                <Crown className="w-4 h-4" /> Get Full Access
              </Link>
              <Link to="/register" className={`inline-flex items-center gap-2 px-6 py-3 ${theme.button?.secondary || 'bg-[var(--bg-secondary)]'} rounded-xl ${theme.text.primary} font-medium transition-all duration-200`}>
