      ...
      <main className="pt-24 pb-20">
        <div className="container max-w-5xl mx-auto px-4">
          ...
          <div className="flex items-center gap-4 mb-8">
            <Link to="/mentee" className={`inline-flex items-center gap-2 text-sm ${theme.text.muted} hover:text-[#f97316] transition-colors`}>
              <ArrowLeft className="w-4 h-4" /> Dashboard
            </Link>
            <span className={`text-sm ${theme.text.muted}`}>/</span>
            <Link to="/learn" className={`text-sm ${theme.text.muted} hover:text-[#f97316] transition-colors`}>
              All Courses
            </Link>
          </div>
          ...
          <div className="text-center mb-14">
            ...
            <Link
              to={`/learn/aws/${firstSlug}`}
              className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-[#f97316] to-[#ea580c] text-white font-bold rounded-xl hover:from-[#ea580c] hover:to-[#c2410c] transition-all duration-300 shadow-lg text-lg"
            >
              Start Learning
          ...
          {!user ? (
            ...
            <div className={`p-8 rounded-2xl ${theme.bg.card} border-2 border-orange-500/20 text-center`}>
              ...
              <h3 className={`text-xl font-bold ${theme.text.primary} mb-2`}>Unlock the AWS Course</h3>
              ...
              {AWS_SECTIONS.map((section) => (
                <div key={section.id}>
                  <div className="flex items-center gap-3 mb-4 flex-wrap">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full font-medium ${sectionBadgeColor[section.color] || sectionBadgeColor.orange}`}>
                      {section.topics.length} topics
                    </span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {sortByDifficulty(section.topics, AWS_TOPICS).map((slug) => {
                      const topic = AWS_TOPICS[slug];
                      if (!topic) return null;
                      return (
                        <Link
                          key={slug}
                          to={`/learn/aws/${slug}`}
                          className={`flex items-start gap-3 p-4 rounded-xl ${theme.bg.card} border ${sectionBorderColor[section.color] || sectionBorderColor.orange} transition-all duration-200 group`}
                        >
                          <div className="flex-1 min-w-0">
                            <p className={`font-medium text-sm ${theme.text.primary} group-hover:text-[#f97316] transition-colors truncate`}>
                              {topic.title}
                            </p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className={`text-xs ${difficultyColor[topic.difficulty] || 'text-[var(--yellow)]'}`}>{topic.difficulty}</span>
                              <span className={`text-xs ${theme.text.muted}`}>· {topic.duration}</span>
                            </div>
                          </div>
                          <ChevronRight className={`w-4 h-4 ${theme.text.muted} group-hover:text-[#f97316] transition-colors shrink-0 mt-1`} />
                        </Link>
                      );
                    })}
                  </div>
